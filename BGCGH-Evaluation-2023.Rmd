...
- Total attendance counts. Grant attendance targets.

- NYOI 2023 Member Surveys possibly haven't been received yet. Check on NYOI age.

- 2023 Spring Grade Westwood doesn't have grade level information.
...
- I still have not been told the outcome of speaking to the school district about the lack of report cards.

    + They say they have grades for every kid unless they transferred.

- I did not receive any family engagement surveys, so I'm assuming we're dropping the supporting families evaluation question from the report.

    + Yes, they're dropped.

- We asked for the final data, but only received grades. We still do not have 2023 attendance data or NYOI 2023 Member Surveys.
    ...
    + Shared space model is any partnership they have. They share buildings with other organizations and schools.
...
- What's the difference between a Club ID and a Comet ID?
...
# UPDATE: with 2023 data, make table with the number of students attending each site for at least 45 days
...
# UPDATE: add caption title to chart (is this the percent of members in the club that attended at least 45 days?)
...
ggplot(attendance.grant.plot, aes(x = Year, y = attendance.grant.plot$`Percent of Members`, color = Site)) +
  ...
  ylab("Percent of Members") +
    scale_fill_hue(l=40) +
  theme(panel.background = element_blank()) +
  geom_text(aes(label = ifelse(attendance.grant.plot$Site == 'Total', scales::percent(attendance.grant.plot$`Percent of Members`) , "")), vjust = 1.5) 

# UPDATE (optional): I feel like the total blends in with the rest on the graph here.
...
*Past report language: A limitation of this analysis is the amount of available report cards. Despite the number of club Members in Texas ACE™, only a sample size of ### report cards were available. Because of the variability in grade level and Club site within the sample, this data has limited generalizability of the actual grade distribution within the five Clubs.*
...
# ACE 2023 Fall is how these were labeled, but that makes no sense. Waiting on clarification.
...
# View(csv.list[[19]]) # Westwood doesn't have grade level information.
...
ace2023 <- csv.list[[17]] %>% 
  mutate(Grade = gsub("1", "1st", Grade),
         ...
         Grade = gsub("7", "7th", Grade),
         Grade = gsub("8", "8th", Grade)) %>% 
  bind_rows(., list(csv.list[[16]], csv.list[[18]], csv.list[[19]], csv.list[[20]]), .id = "id")

ace2023 <- ace2023 %>% 
  mutate(Site = ifelse(substr(ace2023$Club.ID, 1, 3) == "SOM", "Spring Oaks Middle", ""),
         Site = ifelse(substr(ace2023$Club.ID, 1, 3) == "SBE", "Spring Branch Elementary", Site),
         Site = ifelse(substr(ace2023$Club.ID, 1, 3) == "TWH", "The Women's Home", Site),
         Site = ifelse(substr(ace2023$Club.ID, 1, 2) == "WW", "Westwood Elementary", Site),
         Site = ifelse(substr(ace2023$Club.ID, 1, 2) == "WV", "Woodview Elementary", Site),
         Year = as.numeric(2023),
         Term = "Spring",
         Data = "ACE")

ace2023.comet.duplicates <- ace2023$Comet.ID[duplicated(ace2023$Comet.ID)]
# paste(ace2023.comet.duplicates)
# "1826954" "1841066" "1845474" "1853139" "1842531" "1851898" "1845379" "1854408" "1845372" "1852067" "1845368"

ace2023.duplicates <- ace2023$Club.ID[duplicated(ace2023$Club.ID)]
...
*It would be nice to recreate this chart with 2023 data, but we don't have grade level information for Westwood.*
...
ace <- bind_rows(list(ace2021, ace2022, ace2023)) %>%
...
*I'm not sure how to organize the below table. Maybe by year would be best?*
