...
(Last year's report was Fall 2021 - Spring 2022. This year's report will be Fall 2022 - Spring 2023.) 
    ...
    + However, it looks very similar to the Spring 2022 data from last year’s report. (We have the same number of report cards, from the same grades and the same sites, other than Woodview, as the Spring 2021 data in last year's report. Quite the coincidence if it isn't the exact same data)

- 2021 ACE data is only from Spring. Should we also have Fall?

- The ACE folder is named 2018-2022. Are we supposed to grab 2018-2020 data from the old evaluation reports? We could give very basic comparisons like total attendance, but we're limited in what we can get from these reports because the exact same graphs were not recreated each year and most of the graphs do not have individual data labels.
...
- When will we have the 2023 NYOI files? How is this survey administered? Wondering about the number of responses.

- NYOI Member Surveys: Should we only look at the same member experience indicators we did last year? (Emotional Safety, Physical Safety, Fun, Sense of Belonging, Adult Connections, Expectations, and Recognition.)
...
- Notes from 19-20 report: number of days of BGCGH Texas ACE programming attended by a given student could not be calculated to a reasonable level of certainty because of disparities in data collection and management procedures.
    ...
    + I would like some more information about this and if the same is true for this year.

- You sent Youth Program Quality Intervention (YPQI) data. The reports themselves and the past evaluations call it the Youth Program Quality Assessment (YPQA). What do you prefer for this report?

- Supporting parents evaluation questions?
...
# Need updated file from BGCGH -  tab for Westwood is not complete
nyoi.2022.list <- list()
for (site in site.names){
...
# UPDATE: copy and paste above code but for NYOI 2023 file
...
# I'd like to reorder the measures to show highest rated measures on the left and lowest on the right, so it's easy to see at a glance what's in most need of improvement.
...
# But maybe change it to a line graph of optimal over time?
...
## Youth Program Quality Intervention (or Assessment?)
...
ggplot(data = ypqi.sets, aes(x = Measure, y = Score, fill = Score)) + 
  geom_col() + 
  facet_grid(rows = vars(Site), cols = vars(Year)) +
  labs(title = "Program Quality Assessment Scores") +
  guides(x = guide_axis(angle = 90)) +
  geom_text(aes(label = Score), color = "white", vjust = 1.5) +
    theme(panel.background = element_blank())

# Instructional total score should stand out and be at the end
# Color scores below three and above three instead of the current gradient
