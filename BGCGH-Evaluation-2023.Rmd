...
# Import all pdf files. I did remove the old evaluation reports, so these are only the ypqi pdfs.
...
for(pdf in 1:max(row_number(pdf.file.list))){
  ...
  ypqi.table <- ypqi.pdf_table[[4]][(str_which(ypqi.pdf_table[[4]], "Score Set 1")):(str_which(ypqi.pdf_table[[4]], "The"))] #on page 4
...
ypqi <- ypqi %>% 
  mutate(Site = ifelse(str_detect(ypqi$pdf.file.list, "SBE"), "Spring Branch Elementary", Site),
         ...
         Site = ifelse(str_detect(ypqi$pdf.file.list, "TWH"), "The Womans' Home", Site),
