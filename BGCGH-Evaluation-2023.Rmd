...
- NYOI Member Surveys have a lot of different things we could look at. Need to know your priorities. Should we look at the same that we did last year?
...
# Everything we have right now is from 2021 and 2022.

# Summary Report is on pg 4
# Pg 9 says scores lower than 3 may be the best candidates for improvement action


pdf_table <- map(pdf.list[[4]], ~ str_split(.x, "\n") %>% unlist())

#specify the start and end of the table data
table <- pdf_table[[4]][(str_which(pdf_table[[4]], "Score Set 1")):(str_which(pdf_table[[4]], "The"))]  #on page 4
...
colnames(data_table)<- c("Measure", "Score")
...
for(pdf in 5:13){
  ...
  pdf_table <- map(pdf.list[[pdf]], ~ str_split(.x, "\n") %>% unlist())
  ...
  table <- str_replace_all(table," \\s+ ", "|")
  text_con <- textConnection(table)
  data_table1 <- read.csv(text_con, sep = "|")
  
  #Create a list of column names
  colnames(data_table1)<- c("Measure", "Score")
  ...
  ypqi <- bind_rows(data_table, data_table1, .id = "Site")
  
}


# need to save results from loop in list, then bind_rows
...
nyoi.2021 <- bind_rows(xlsx.list[[2]], .id = "Site")
  ...
  pdf_table <- map(pdf.list[[5]], ~ str_split(.x, "\n") %>% unlist())
...
#specify the start and end of the table data
table_start <- pdf_table[[4]] %>% 
  str_which("Score Set 1")
table_end <- pdf_table[[4]] %>% 
  str_which("The")

#Build the table and remove special characters
table <- pdf_table[[4]][(table_start):(table_end)] #on page 4
table <- str_replace_all(table," \\s+ ", "|")
text_con <- textConnection(table)
data_table <- read.csv(text_con, sep = "|")
...
kable(experience, caption = "Overall Club Experience by Site, 2021", digits = 2)
