...
Boys & Girls Clubs of America’s National Youth Outcomes Initiative (NYOI) - Club Member Experience
...
# UPDATE if file numbers are different
# NYOI 2021 Member Survey Report
...
# Last year we only looked at Emotional Safety, Physical Safety, Fun, Sense of Belonging, Adult Connections, Expectations, and Recognition.
...
*Why do we have so few survey responses?*

```{r}


nyoi.2021.responses <- nyoi.2021 %>% 
  filter(Measure == "# of Member Responses") %>% 
  mutate("# of Member Responses" = as.numeric(...2)) %>%
  select(Year, Site, "# of Member Responses") %>% 
  group_by(Year) %>% 
  summarise("# of Member Responses" = sum(`# of Member Responses`))

kable(nyoi.2021.responses)
...
test <- as.data.frame(colnames(nyoi.2022))

nyoi.columns <- c(paste(subset(test, str_starts(test$`colnames(nyoi.2022)`, "Detail"), select = `colnames(nyoi.2022)`)))


nyoi.2022.clean <- nyoi.2022 %>% 
  mutate(Measure = paste0(c(nyoi.columns)))
