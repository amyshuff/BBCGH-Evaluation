...
ggplot(club.gender.plot, aes(x = Site, y = demographic.pct)) +
  ...
  coord_flip() +
  geom_text(aes(label = scales::percent(demographic.pct)), color = "black", hjust = 1.5, position = "stack") +
  labs(title = "Gender") +
  theme(panel.background = element_blank(),
...
ggplot(club.age.plot, aes(x = forcats::fct_rev(Site), y = demographic.pct)) +
  geom_col(aes(fill = Demographic)) +
  coord_flip() +
  geom_text(aes(x = Site, y = demographic.pct, label = scales::percent(demographic.pct), group = Demographic),
...
```{r Math GPA over time, fig.height=4, fig.width=10}
...
ggplot(ace.averages, aes(x = Site, y = Math, fill = factor(ace.averages$Year))) +
  ...
  geom_text(aes(label = Math), 
            position = position_dodge(width = .9),
            vjust = 2) +
  labs(title = "Math", subtitle = "Average GPA by Site and Year") +
  theme(panel.background = element_blank()) +
  ylab("Average Math GPA") +
  xlab("") +
  guides(fill = guide_legend(title = "Year")) +
...
ggplot(ace.averages, aes(x = Site, y = English.Reading, fill = factor(ace.averages$Year))) +
  ...
  geom_text(aes(label = English.Reading), 
            position = position_dodge(width = .9),
            vjust = 2) +
  labs(title = "English/Reading", subtitle = "Average GPA by Site and Year") +
  theme(panel.background = element_blank()) +
  ylab("Average English/Reading GPA") +
  xlab("") +
  guides(fill = guide_legend(title = "Year")) +
...
print(  
ggplot(data = subset(nyoi.experience, Site == name), 
       aes(x = Measure, y = Value, fill = Score)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_grid(~Year) +
  labs(y = "Percent Response",
       title = paste(name), subtitle = "Club Experience Measures") +
  guides(x = guide_axis(angle = 90)) +
  scale_y_continuous(labels = scales::label_percent(accuracy = 1)) +
  scale_fill_brewer(name = "", labels = c("Needs Improvement", "Fair", "Optimal"), palette = "Blues") +
    geom_text(aes(label = ifelse(Score == "Optimal", scales::percent(Value, accuracy = 1), '')), 
              color = "black", vjust = 1.5, position = "stack") +
    theme(panel.background = element_blank(),
...
ggplot(data = nyoi.overall, aes(x = Measure, y = Value, fill = Score)) + 
  ...
  scale_fill_brewer(name = "", labels = c("Needs Improvement", "Fair", "Optimal"), palette = "Blues") +
  geom_text(aes(label = ifelse(Score == 'Optimal' | Score == 'Fair', scales::percent(Value), "")), color = "black", vjust = 1.5, position = "stack") +
  theme(panel.background = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) +
  xlab("")
...
# ggplot(data = ypqi.sets, aes(x = Measure, y = Score, fill = Score >= 5)) + 
#   geom_col() + 
#   facet_grid(rows = vars(Site), cols = vars(Year)) +
...
#   theme(panel.background = element_blank()) +
#   scale_fill_brewer(name = "Score", labels = c("Score of 5", "Less than 5"), palette = "Blues")
...
# ggplot(data = subset(ypqi.sets, Site == 'Spring Branch Elementary'), aes(x = Measure, y = Score, fill = Year)) +
#   geom_bar(stat = "identity", position = "dodge") +
#   geom_text(aes(label = Score), 
#             position = position_dodge(width = .9),
#             vjust = 2) +
#   labs(title = "Spring Branch Elementary", subtitle = "Program Quality Assessment Scores") +
#   theme(panel.background = element_blank()) +
#   scale_fill_brewer(palette = "Blues")

ypqi.names <- c("Spring Branch Elementary", "Spring Oaks Middle", "The Women's Home", "Woodview Elementary", "Westwood Elementary") 
  
  
for (name in ypqi.names) {
print(  
ggplot(data = subset(ypqi.sets, Site == name), aes(x = Measure, y = Score, fill = Year)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = Score), 
            position = position_dodge(width = .9),
            vjust = 2) +
  labs(title = paste(name), subtitle = "Program Quality Assessment Scores") +
  theme(panel.background = element_blank(),
        axis.text = element_text(color = "black")) +
